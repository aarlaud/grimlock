...
trait Aggregator[P <: HList, S <: HList, Q <: HList] extends AggregatorWithValue[P, S, Q] { self =>
   ...
   */
  override def withPrepare[D](preparer: (Cell[P]) => Content[D]) = new Aggregator[P, S, Q] {
    type T = self.T
    type O[A] = self.O[A]
   ...
   */
  override def andThenMutate[D](mutator: (Cell[Q]) => Content[D]) = new Aggregator[P, S, Q] {
    type T = self.T
    type O[A] = self.O[A]
...
trait AggregatorWithValue[P <: HList, S <: HList, Q <: HList] extends java.io.Serializable { self =>
   ...
   * @return An aggregator that prepares the content and then runs `this`.
   */
  def withPrepare[D](preparer: (Cell[P]) => Content[D]) = new AggregatorWithValue[P, S, Q] {
    type T = self.T
    type V = self.V
    type O[A] = self.O[A]
   ...
   * @return An aggregator that runs `this` and then updates the resulting contents.
   */
  def andThenMutate[D](mutator: (Cell[Q]) => Content[D]) = new AggregatorWithValue[P, S, Q] {
    type T = self.T
    type V = self.V
    type O[A] = self.O[A]
   ...
   * @return An aggregator that prepares the content and then runs `this`.
   */
  def withPrepareWithValue[D](preparer: (Cell[P], V) => Content[D]) = new AggregatorWithValue[P, S, Q] {
    type T = self.T
    type V = self.V
    type O[A] = self.O[A]
   ...
   * @return An aggregator that runs `this` and then updates the resulting contents.
   */
  def andThenMutateWithValue[D](mutator: (Cell[Q], V) => Content[D]) = new AggregatorWithValue[P, S, Q] {
    type T = self.T
    type V = self.V
    type O[A] = self.O[A]
